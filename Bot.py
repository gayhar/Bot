...
USERS_FILE = os.path.join(BASE_DIR, "Users.json")  # مستقیم در روت
BANNED_FILE = os.path.join(BASE_DIR, "Banned.json")  # مستقیم در روت

# فایل‌های سوالات
QUESTIONS_FILES = {
    "7": os.path.join(BASE_DIR, "Questions7.json"),
    "8": os.path.join(BASE_DIR, "Questions8.json"), 
    "9": os.path.join(BASE_DIR, "Questions9.json"),
}
...
def is_banned(telegram_id):
    if is_admin(telegram_id):  # ادمین‌ها هرگز مسدود نیستند
        return False
    banned = load_banned()
...
def start_command(message):
    ...
    # اگر ادمین هست، مستقیم به منوی اصلی برو
    if is_admin(telegram_id):
        user = get_user(telegram_id)
        if not user:
            # اگر ادمین ثبت‌نام نکرده، خودکار ثبت‌نام کن
            fullname = message.from_user.first_name
            if message.from_user.last_name:
                fullname += " " + message.from_user.last_name
            username = message.from_user.username or "ندارد"
            
            if add_user(telegram_id, fullname, username, "7"):
                print(f"✅ ادمین خودکار ثبت‌نام شد: {fullname}")
        
        bot.send_message(telegram_id, f"👑 سلام ادمین عزیز! 😊", reply_markup=main_menu())
        return
...
def process_name(message):
    ...
    # اگر ادمین هست، مستقیم برو به منو
    if is_admin(telegram_id):
        pending_registration.pop(telegram_id, None)
...
def process_grade(message):
    ...
    if text == "🔙 بازگشت":
        pending_registration.pop(telegram_id, None)
        bot.send_message(telegram_id, "👋 لطفاً نام و نام خانوادگی خود را وارد کنید:")
        pending_registration[telegram_id] = {"step": "name"}
        return
    ...
    user_data = pending_registration.get(telegram_id)
    if not user_data:
        bot.send_message(telegram_id, "❌ خطا در ثبت‌نام! لطفاً دوباره /start را بزنید.")
        return
    ...
    if add_user(telegram_id, fullname, username, grade):
        ...
        print(f"✅ کاربر جدید ثبت‌نام شد: {fullname} (آیدی: {telegram_id})")
    else:
...
def handle_all_messages(message):
    ...
    # اگر ادمین هست، مسدودیت بررسی نشود
    if not is_admin(telegram_id) and is_banned(telegram_id):
        bot.send_message(telegram_id, "🚫 شما از این بات مسدود شده‌اید!")
        return
    
    # اگر در حال ثبت‌نام هست، پردازش کن
    if telegram_id in pending_registration:
        return
    ...
    if is_admin(telegram_id):
        if text.startswith("+ "):
            ...
            if delete_user(fullname):
                bot.send_message(telegram_id, f"✅ کاربر {fullname} حذف شد.")
            else:
                bot.send_message(telegram_id, "❌ کاربر یافت نشد.")
            return
            ...
            # مسدود کردن: Ban علی
            fullname = text[4:].strip()
            if ban_user(fullname):
                bot.send_message(telegram_id, f"✅ کاربر {fullname} مسدود شد.")
            else:
                bot.send_message(telegram_id, "❌ کاربر یافت نشد.")
            return
            ...
            # آزاد کردن: Unban علی
            fullname = text[6:].strip()
            if unban_user(fullname):
                bot.send_message(telegram_id, f"✅ کاربر {fullname} آزاد شد.")
            else:
                bot.send_message(telegram_id, "❌ کاربر یافت نشد یا مسدود نبود.")
            return
...
if __name__ == "__main__":
    print("\n" + "="*50)
    print("🚀 بات آموزشی فارسی - نسخه نهایی")
    print("="*50)
